...
(function () {
  ...
  function initStatsCounter() {
    ...
    // Safety net: if a fast scroll (or any other timing quirk) ever
    // causes the observer to miss triggering, force every remaining
    // counter to its final value after a short delay — a stat card
    // can never be left permanently stuck at "0".
    setTimeout(() => {
      counters.forEach((el) => {
        if (el.textContent === "0") {
          animateCounter(el);
